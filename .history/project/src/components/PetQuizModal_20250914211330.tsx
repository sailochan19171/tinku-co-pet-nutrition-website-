...
const PetQuizModal: React.FC<Props> = ({ open, onClose }) => {
  ...
  const recommendation = useMemo(() => {
    ...
    // Breed-specific image selection with sensible defaults
    const breedKey = (info.breed || '').trim().toLowerCase();
    const breedImages: Record<string, string> = {
      // Dogs
      'german shepherd': 'https://images.pexels.com/photos/333083/pexels-photo-333083.jpeg?auto=compress&cs=tinysrgb&w=800',
      'labrador retriever': 'https://images.pexels.com/photos/8700/wall-animal-dog-pet.jpg?auto=compress&cs=tinysrgb&w=800',
      'golden retriever': 'https://images.pexels.com/photos/2253275/pexels-photo-2253275.jpeg?auto=compress&cs=tinysrgb&w=800',
      'bulldog': 'https://images.pexels.com/photos/164186/pexels-photo-164186.jpeg?auto=compress&cs=tinysrgb&w=800',
      'poodle': 'https://images.pexels.com/photos/1458916/pexels-photo-1458916.jpeg?auto=compress&cs=tinysrgb&w=800',
      'beagle': 'https://images.pexels.com/photos/460823/pexels-photo-460823.jpeg?auto=compress&cs=tinysrgb&w=800',
      // Cats
      'persian': 'https://images.pexels.com/photos/208773/pexels-photo-208773.jpeg?auto=compress&cs=tinysrgb&w=800',
      'siamese': 'https://images.pexels.com/photos/1663411/pexels-photo-1663411.jpeg?auto=compress&cs=tinysrgb&w=800',
      'maine coon': 'https://images.pexels.com/photos/2194261/pexels-photo-2194261.jpeg?auto=compress&cs=tinysrgb&w=800',
      'bengal': 'https://images.pexels.com/photos/1440404/pexels-photo-1440404.jpeg?auto=compress&cs=tinysrgb&w=800',
    };
    const defaultImage = info.type === 'Cat' ? '/muscles-cat.png' : '/hero-dog.png';
    const image = breedImages[breedKey] || defaultImage;
        ...
        {step === 'result' && (
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    setError(null);
                    setCreating(true);
                    const species = info.type === 'Cat' ? 'cat' : 'dog';
                    const weightMatch = info.weight?.match(/\d+(?:\.\d+)?/);
                    const weightKg = weightMatch ? parseFloat(weightMatch[0]) : 10;
                    const ageYears = info.age === 'Puppy/Kitten' ? 0.5 : info.age === 'Senior' ? 8 : 3;
                    const activityLevel = info.activity === 'High' ? 'high' : info.activity === 'Low' ? 'low' : 'moderate';
                    const allergies = info.concerns.includes('None') ? [] : info.concerns;

                    const petRes = await fetch('/api/pets', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
                        name: info.name?.trim() || 'Pet',
                        species,
                    ...
                    // Close modal and navigate to Home meal plan anchor
                    onClose();
                    navigate('/#meal-plan');
                  } catch (e: any) {
              ...
              >
                {creating ? 'Creating...' : 'Create Pet & Generate Plan'}
              </button>
