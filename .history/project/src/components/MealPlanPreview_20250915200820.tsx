    ...
    <section className="section-premium container mx-auto px-4">
      <div className="bg-white rounded-xl shadow border p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
          <h2 className="text-2xl font-semibold text-orange-600 text-orange-600">Your Meal Plan</h2>
          {firstPet && (
            <button
        ...
        {!loading && firstPet && plan && (
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-2 text-gray-700">
              <span>Week:</span>
              <span className="font-medium text-orange-600 text-orange-600">{plan.weekISO}</span>
              <span className="mx-1">•</span>
              <span>Pet:</span>
              <span className="font-medium text-orange-600 capitalize text-orange-600 capitalize">{firstPet.name}</span>
              {selectedDay && (
                <>
                  <span className="mx-1">•</span>
                  <span>Day:</span>
                  <span className="font-medium text-orange-600 text-orange-600">{formatDayLabel(selectedDay.dateISO)}</span>
                  <span className="mx-1">•</span>
                  <span>Total:</span>
                  <span className="font-semibold text-orange-600 text-orange-600">{dayTotal} g/day</span>
                </>
              )}
            </div>
                ...
                {selectedDay.items?.map((it, idx) => {
                  ...
                  return (
                    <div key={idx} className="border rounded p-4">
                      ...
                      <div className="h-2 w-full bg-orange-500 rounded">
                        <div
